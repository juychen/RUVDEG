...
| **UV latent PCA (w)** | **按 company 清晰分成 3 簇** → w 确实捕获了技术批次 ✅ |
| **Reconstruction** | 点紧贴 y=x 对角线 → 重建良好 |

> **最关键的一张是 UV latent PCA**：`w` 的两个主成分把 beirui / seekgene / yunzhun
> 三家公司分得干干净净，而 `z` 的 PCA 里两个 status 是重叠的。
> 这正是我们想要的分工：**w 管技术、z 管生物**。
...
> （NC 跨 donor 波动 ↓ 到 **0.244×**，UV 潜空间按公司完美分离）。
